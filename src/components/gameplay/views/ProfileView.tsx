  ...
  return (
    <div className="min-h-screen flex flex-col animate-fade-in">
      ...
      <div className="flex-1 px-5 pb-24 overflow-y-auto scrollbar-hide">
        <div className="night-card p-5 mb-4 text-center">
          <PortraitAvatar target={playerCharacter} size="xl" era={story.era} />
          <h3 className="text-xl font-bold mt-3">{playerCharacter.name}</h3>
          <p className="text-sm text-ink-4">{playerCharacter.role}</p>
        </div>

        {hasAmnesia && (
          <div className="night-card p-4 mb-4 border-cinnabar/30">
            <h4 className="font-bold text-sm text-cinnabar-2 mb-2">模糊記憶</h4>
            <p className="text-sm text-paper-3 italic">{toFirstPerson(playerCharacter.secretPrologue || '')}</p>
            <p className="text-xs text-ink-4 mt-2">有些記憶像是被水泡過的照片——你能看見輪廓，卻看不清細節。</p>
          </div>
        )}

        {hasAmnesia && unlockedMemories.length > 0 && (
          <div className="night-card p-4 mb-4 border-purple-800/30">
            <h4 className="font-bold text-sm text-purple-400 mb-2">恢復的記憶</h4>
            <div className="space-y-2">
              {unlockedMemories.map((mem, i) => (
                <p key={i} className="text-sm text-paper-3 border-l-2 border-purple-700/30 pl-3">
        ...
        <div className="night-card p-4 mb-4">
          <h4 className="font-bold text-sm text-cinnabar-2 mb-2">{hasAmnesia ? '隱藏的秘密' : '我的秘密'}</h4>
          <p className={`text-sm text-paper-3 ${hasAmnesia && unlockedMemories.length < 3 ? 'blur-sm select-none' : ''}`}>
            {toFirstPerson(playerCharacter.secret)}
          </p>
          {hasAmnesia && unlockedMemories.length < 3 && (
            <p className="text-xs text-ink-4 mt-2">收集更多線索來恢復記憶...</p>
          )}
        </div>

        {playerCharacter.tool && (
          <div className="night-card p-4 mb-4 border border-indigo-800/30">
            <h4 className="font-bold text-sm text-indigo-400 mb-2">隨身道具</h4>
            <p className="text-sm font-medium text-paper">{playerCharacter.tool.name}</p>
            <p className="text-xs text-ink-4 mt-1">{playerCharacter.tool.description}</p>
          </div>
        )}

        <div className="night-card p-4 mb-4">
          <h4 className="font-bold text-sm text-red-400 mb-2">我的動機</h4>
          <p className="text-sm text-paper-3">{toFirstPerson(playerCharacter.motive)}</p>
        </div>

        <div className="night-card p-4 mb-4">
          <h4 className="font-bold text-sm text-verdigris-2 mb-2">我的不在場證明</h4>
          <p className="text-sm text-paper-3">{toFirstPerson(playerCharacter.alibi)}</p>
        </div>

        <h3 className="font-bold text-sm text-ink-4 mb-3">其他角色</h3>
        <div className="space-y-2">
          {story.characters.filter((c) => c.id !== playerCharacter.id && c.isPlayable).map((char) => (
            <div key={char.id} className="night-card p-3 flex items-center gap-3">
              <PortraitAvatar target={char} size="sm" era={story.era} />
              <div>
                <span className="font-bold text-sm">{char.name}</span>
                <p className="text-xs text-ink-4">{char.role}</p>
              </div>
            </div>
          ))}
        </div>
