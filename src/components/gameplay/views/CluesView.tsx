    ...
    <div className="min-h-screen flex flex-col animate-fade-in">
      <div className="px-5 pt-5 pb-3">
        <div className="flex items-center justify-between mb-3">
          <button
            onClick={onBack}
            className="flex items-center gap-1 text-ink-4 text-sm hover:text-paper"
          >
            <span>←</span>
            返回
          </button>
          <span className="text-xs text-ink-4">{myClues.length} / {story.clues.length}</span>
        </div>
        <h2 className="text-xl font-bold flex items-center gap-2">
          <Package className="w-5 h-5 text-cinnabar-2" />
          線索背包
        </h2>
      </div>
          ...
          <div className="space-y-3">
            {myClues.map((clue) => {
              ...
              return (
                <div
                  key={clue.id}
                  className="night-card p-4 border-l-2 border-cinnabar-2/50"
                >
                  <button
                    onClick={() => {
                      if (isExpanded) {
                  ...
                  >
                    <div className="flex items-center gap-2 mb-1 flex-wrap">
                      <Bookmark className="w-3.5 h-3.5 text-cinnabar-2" />
                      <h4 className="font-bold text-sm">{clue.name}</h4>
                      <span className="text-[10px] px-1.5 py-0.5 bg-night-3 text-ink-4 rounded-full capitalize">
                      ...
                      {clue.details && clue.details.length > 0 && (
                        <span className="ml-auto text-[10px] text-ink-4 flex items-center gap-0.5">
                          {isExpanded ? <ChevronUp className="w-3 h-3" /> : <ChevronDown className="w-3 h-3" />}
                          {clue.details.length} 處細節
                        </span>
                      )}
                    </div>
                    <p className="text-sm text-paper-3 leading-relaxed">{clue.description}</p>
                    <p className="text-xs text-ink-4 mt-2">發現地點：{story.locations.find((l) => l.id === clue.locationId)?.name}</p>
                  </button>
                  ...
                  {isPlayerKiller && (
                    <div className="mt-2 flex items-center gap-2 flex-wrap">
                      ...
                      <button
                        ...
                        className="flex items-center gap-1 text-xs text-verdigris-2/70 hover:text-verdigris-2 transition-colors px-2 py-1 rounded-lg hover:bg-verdigris-deep/30"
                      >
                        <Target className="w-3 h-3" />
                        栽贓給…
                      </button>
                      {getFramedTarget(clue.id) && (
                        <span className="text-[10px] px-1.5 py-0.5 bg-verdigris-deep/60 text-verdigris-2 border border-verdigris/40 rounded-full flex items-center gap-1">
                  ...
                  {isExpanded && clue.details && clue.details.length > 0 && (
                    <div className="mt-3 space-y-2 pt-3 border-t border-night-3/50 animate-slide-up">
                      {clue.details.map((detail, idx) => (
                        <div key={idx} className="bg-night-2/40 rounded-lg p-3">
                          <h5 className="text-xs font-bold text-cinnabar-2 mb-1">{detail.label}</h5>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 animate-fade-in">
          ...
          <div className="relative z-10 w-full max-w-sm glass rounded-2xl p-5 animate-slide-up">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-sm font-bold text-paper">選擇栽贓對象</h3>
              <button onClick={() => setFramingClueId(null)} className="p-1.5 rounded-lg hover:bg-night-3/60">
            ...
            <div className="space-y-2">
              {story.npcs.map((npc) => (
                <button
                  ...
                  className="w-full flex items-center gap-3 p-3 rounded-xl bg-night-3/40 hover:bg-night-3/70 border border-ink-3/30 transition-all text-left"
                >
                  <span className="text-lg">{npc.avatar}</span>
                  <div>
                    <p className="text-sm font-medium text-paper">{npc.name}</p>
                    <p className="text-xs text-ink-4">{npc.role}</p>
                  </div>
                  <span className="ml-auto text-xs text-verdigris-2/70">
