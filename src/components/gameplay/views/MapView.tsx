    ...
    <div className="min-h-screen flex flex-col animate-fade-in">
      <div className="px-5 pt-5 pb-3 safe-top">
        ...
        <div className="flex items-center gap-1.5 mb-2 text-[11px] text-ink-4">
          ...
          {timeIcon}
          <span className="text-ink-4 italic">{getTimeAtmosphere(gameTime.period)}</span>
        </div>
        <div className="flex items-center justify-between mb-3">
          <h2 className="font-kai text-xl">{story.title}</h2>
          <div className="flex items-center gap-2">
            <button
              onClick={onShowProfile}
              className="p-2 rounded-xl night-card"
            >
            ...
            return (
              <button
                ...
                className={`relative p-4 rounded-2xl border text-left transition-all hover:scale-[1.02] active:scale-[0.98] ${
                  isVisited
                    ? 'bg-night-3/60 border-ink-3/50'
                    : 'bg-night-2/40 border-night-3/50'
                }`}
              >
                ...
                <div className="flex items-center gap-1.5 mt-1.5">
                  ...
                  {hasClue && (
                    <span className="text-[10px] px-1.5 py-0.5 bg-cinnabar-deep/60 text-cinnabar-2 rounded-full">
                      線索
                    </span>
                  )}
                  {isVisited && !hasClue && (
                    <span className="text-[10px] px-1.5 py-0.5 bg-night-3 text-ink-4 rounded-full">
                      已探索
                    </span>
                  )}
                </div>
      ...
      {/* 底部導航 */}
      <div className="fixed bottom-0 left-0 right-0 p-4 pb-[max(1rem,env(safe-area-inset-bottom))] bg-gradient-to-t from-night via-night to-transparent">
        <div className="flex gap-3">
          <button
            onClick={onShowClues}
            className="flex-1 py-3 rounded-xl night-card flex items-center justify-center gap-2 text-sm font-medium hover:bg-night-3/80"
          >
